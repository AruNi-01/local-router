...
- 每个服务都想占一个端口，端口冲突会变成日常问题
- 服务重启、切分支、切 workspace 之后，访问地址不断变化
- 同一个项目开了多个 git worktree 时，本地端口会互相冲突，也很难判断当前到底是哪一个分支在提供服务
- 前端、API、文档站、worker、桌面端混在同一个仓库里，但运行方式并不一样
- 进程状态、日志、路由、健康检查散落在终端、浏览器标签页和零散脚本里

LocalRouter 的目标就是把这套环境重新变得可预测。

它为每个可运行服务提供受控的本地运行时、稳定的路由地址，以及一套由 daemon、dashboard 和 CLI 共享的控制平面。开发者不需要再记“今天这个应用跑在什么端口”，也不需要再猜“当前到底是哪一个 worktree / 分支在提供服务”；LocalRouter 会分配内部端口、保持对外访问地址稳定、统一代理请求，并把真实运行状态集中展示出来。

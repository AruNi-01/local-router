...
function servicesFromManifest(
  projectId: string,
  existingManifestYaml: string,
): ServiceFormData[] {
  try {
    const parsed =
      (parseYaml(existingManifestYaml) as {
        services?: Record<string, Record<string, unknown>>;
      }) || {};
    return Object.entries(parsed.services || {}).map(([name, raw], index) => ({
      id: `manifest-svc-${index}-${name}`,
      projectId,
      name,
      command: String(raw.command || ""),
      protocol: String(
        raw.protocol || (raw.route === "none" ? "none" : "http"),
      ),
      adapter: String(raw.adapter || "generic"),
      route: String(raw.route || name),
      healthcheck: String(raw.healthcheck || ""),
      language: String(raw.language || "generic"),
      ...
      dependsOn: Array.isArray(raw.depends_on)
        ? raw.depends_on.map((value) => String(value))
        : [],
      enabled:
...
function buildManifest(
  ...
  try {
    existing =
      (parseYaml(existingManifestYaml) as Record<string, unknown>) || {};
  } catch {
    /* use defaults if existing manifest is invalid */
  }
    ...
    services: Object.fromEntries(
      services.map((service) => [
        service.name,
        {
          ...
          env:
            Object.keys(service.env || {}).length > 0 ? service.env : undefined,
          depends_on:
...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"project" | "global" | "manifest">(
    "project",
  );
  ...
  useEffect(() => {
    ...
    setServices(servicesFromManifest(detail.project.id, detail.manifest));
  }, [detail]);
  ...
  return (
    <DashboardLayout>
      <div className="p-8 max-w-[1000px] space-y-6">
        ...
        >
          <div
            className="flex gap-1.5 rounded-lg bg-muted p-1 mb-6"
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <div className="flex h-10 items-center rounded-md border border-input px-3">
                      ...
                      <span className="ml-3 text-sm text-muted-foreground">
                        {service.enabled
                          ? "Service can be started and routed"
                          : "Service stays defined but cannot be started"}
                      </span>
                    </div>
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleServiceChange(index, "adapter", e.target.value)
                      }
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleServiceChange(index, "protocol", e.target.value)
                      }
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      value={service.route}
                      onChange={(e) =>
                        handleServiceChange(index, "route", e.target.value)
                      }
                    />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleServiceChange(index, "language", e.target.value)
                      }
                    />
                  </div>
                </div>
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setConfig({ ...config, apiPort: Number(e.target.value) })
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setConfig({ ...config, proxyPort: Number(e.target.value) })
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  value={config.dnsSuffix}
                  onChange={(e) =>
                    setConfig({ ...config, dnsSuffix: e.target.value })
                  }
                />
              </div>
              <div className="space-y-2">
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Generate manifest draft from project files
                  </p>
                </div>
                <Switch
                  checked={config.autoDetect}
                  onCheckedChange={(checked) =>
                    setConfig({ ...config, autoDetect: checked })
                  }
                />
              </div>
              <Separator />
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Refresh dashboard data when daemon emits updates
                  </p>
                </div>
                <Switch
                  checked={config.hotReload}
                  onCheckedChange={(checked) =>
                    setConfig({ ...config, hotReload: checked })
                  }
                />
              </div>
              <Separator />
        ...
        <div className="flex justify-end">
          <Button
            onClick={handleSave}
            disabled={saveConfig.isPending || saveManifest.isPending}
          >
